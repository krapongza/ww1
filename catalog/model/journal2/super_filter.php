...
class ModelJournal2SuperFilter extends Model {
    ...
    private static function cmpAttrs($a, $b) {
        if(is_numeric($a['text']) && is_numeric($b['text'])){
            return ((int)$a['text'] < (int)$b['text']) ? -1 : 1;
        }
    ...
    private function addFilters($data, $query = ""){
        ...
        $sql .= " AND EXISTS (SELECT * FROM `" . DB_PREFIX . "product_to_store` p2s WHERE p.product_id = p2s.product_id AND p2s.store_id = '" . (int)$this->config->get('config_store_id') . "')";
