...
class ModelCheckoutOrder extends Model {	
	public function addOrder($data) {
		...
		foreach ($data['totals'] as $total) {
			...
			if($this->db->escape($total['code']) == 'total'){
				$this->db->query("INSERT INTO " . DB_PREFIX . "order_total SET order_id = '" . (int)$order_id . "', code = '" . $this->db->escape($total['code']) . "', title = '" . $this->db->escape($total['title']) . "', text = '" . ((float)$total['value'] + floatval( "0.".$order_id)) . "', `value` = '" . ((float)$total['value'] + floatval( "0.".$order_id)) . "', sort_order = '" . (int)$total['sort_order'] . "'");
			}else{
		...
		$this->db->query("INSERT INTO " . DB_PREFIX . "order_total SET order_id = '" . (int)$order_id . "', code = 'satang', title = 'Satang ', text = '" . floatval( "0.".$order_id)  . "', `value` = '" . floatval( "0.".$order_id)  . "', sort_order = '1'");
	...
	public function addPaysbuy($order_id) {
			$this->db->query("UPDATE `" . DB_PREFIX . "order` SET payment_method = 'PAYSBUY', payment_code='paysbuy', date_modified = NOW() WHERE order_id = '" . (int)$order_id . "'");
	}
	...
	echo count($query->row);
		if(count($query->row) > 0){
			//Rollback
			$balance = $credit_query->row['credit'];
			$credit_amount = $balance + $query->row['credit'];

			$this->db->query("UPDATE `" . DB_PREFIX . "customer` SET credit = '" . $credit_amount . "'  WHERE customer_id = '" . (int)$this->customer->getId() . "'");
			//Order_id, total_credit_used, old_credit,							used_or_add[1=used,0=add] , admin_name ='' , status[1=active,0=cancel] , remark
			$this->model_checkout_pointcredit->historyCredit($order_id, $query->row['credit'] , $balance , 0, '' , 1 , 'order cancelled' );
		}
	}
