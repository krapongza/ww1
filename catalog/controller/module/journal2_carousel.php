...
class ControllerModuleJournal2Carousel extends Controller {
    ...
    public function index($setting) {
        ...
        if ($cache === null || self::$CACHEABLE !== true) {
            ...
            /* image dimensions */
            $this->data['image_width'] = 200; //Journal2Utils::getProperty($module_data, 'image_width', $this->config->get('config_image_product_width'));
            $this->data['image_height'] = 300; //Journal2Utils::getProperty($module_data, 'image_height', $height = $this->config->get('config_image_product_height'));
            ...
            switch (Journal2Utils::getProperty($section, 'section_type')) {
                case 'module':
                    switch (Journal2Utils::getProperty($section, 'module_type')) {
                        case 'featured':
                            $products = $this->model_journal2_product->getFeatured($limit, Journal2Utils::getProperty($section, 'filter_category', 0) !== null);
                            break;
                        case 'bestsellers':
            ...
            foreach ($products as $product) {
                ...
                if (!$this->journal2->html_classes->hasClass('mobile') && count($results) > 0) {
                   // $image2 = $this->getImage($results[0]['image'] ? $results[0]['image'] : 'data/journal2/no_image_large.jpg', $this->data['image_width'], $this->data['image_height'], $this->data['image_resize_type']);
					$image2 = $this->getImage($results[0]['image'] ? $results[0]['image'] : 'data/journal2/no_image_large.jpg', 200, 300, $this->data['image_resize_type']);
                }
                ...
                $product_data = array(
                    ...
                    'thumb'         => $image,
                    'thumb2'        => $image2,
					'status_id'		=> $product['stock_status_id'],
                    'name'          => $product['name'],
                    'price'         => $price,
                    'special'       => $special,
                    'date_end'      => $date_end,
