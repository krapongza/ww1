    ...
    protected function getFilters($module_id, $ajax = FALSE) {
            ...
            foreach ($result['values'] as $value) {
                $values[] = array(
                  ...
                  //  'option_value_name'	=> trim($value['option_value_name']) . ' (' . $value['total'] . ')',
				    'option_value_name'	=> trim($value['option_value_name']) ,
		...
		*/
        ...
        if ($this->data['price_filter']) {
            $this->template = 'journal2/template/journal2/module/super_filter_price.tpl';
            $filter_groups[] = array(
        ...
        $this->data['filter_groups'] = $filter_groups; //Journal2Utils::sortArray($filter_groups);
    ...
    private function parseUrl(){
        ...
        foreach (explode("/", $this->request->post['filters']) as $fragment) {
            ...
            if (preg_match($pattern, $fragment)) {
                ...
                $attributes[$attribute_id] = explode(",", $text);
            }
    ...
    public function products() {
        ...
        foreach ($results as $result) {
            ...
            if (count($results) > 0) {
				...
				$image2 = $this->model_tool_image->resize($results[0]['image'] ? $results[0]['image'] : 'data/journal2/no_image_large.jpg', 200, 300);
            }
