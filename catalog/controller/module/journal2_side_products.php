...
class ControllerModuleJournal2SideProducts extends Controller {
    ...
    public function index($setting) {
        ...
        if ($cache === null || self::$CACHEABLE !== true) {
            ...
            $todays_specials =
                Journal2Utils::getProperty($module_data, 'section_type') === 'module' &&
                Journal2Utils::getProperty($module_data, 'module_type') === 'specials' &&
                Journal2Utils::getProperty($module_data, 'todays_specials_only') === '1';
            $limit = $todays_specials ? PHP_INT_MAX : Journal2Utils::getProperty($module_data, 'items_limit', 5);
            ...
            switch (Journal2Utils::getProperty($module_data, 'section_type')) {
                case 'module':
                    switch (Journal2Utils::getProperty($module_data, 'module_type')) {
                        case 'featured':
                            $products = $this->model_journal2_product->getFeatured($limit, Journal2Utils::getProperty($module_data, 'filter_category', 0) !== null);
                            break;
                        case 'bestsellers':
            ...
            foreach ($products as $product) {
                ...
                if (!$this->journal2->html_classes->hasClass('mobile') && count($results) > 0) {
					...
					$image2 = $this->getProductImage($results[0]['image'] ? $results[0]['image'] : 'data/journal2/no_image_large.jpg', 200, 300, $this->data['image_resize_type']);
                }

                if ($special) {
                    $date_end = $this->model_journal2_product->getSpecialCountdown($product['product_id']);
                    if ($todays_specials && date('Y-m-d', strtotime(date('Y-m-d') . "+1 days")) !== $date_end) {
                        continue;
                    }
                }
            ...
            $this->template = "journal2/template/journal2/module/side_products.tpl";
