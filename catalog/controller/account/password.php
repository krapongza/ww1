	...
	private $error = array();
  	...
  	public function index() {	
		...
		if (isset($this->error['warning'])) { 
			$this->data['error_warning'] = 'password ปัจจุบันไม่ถูกต้อง'; //$this->error['warning'];
		} else {
  	...
  	protected function validate() {
    	...
    	if ($this->request->post['oldpassword'] == $this->request->post['password']) {
      		$this->error['password'] = 'password ใหม่ห้ามซ้ำ password ปัจจุบัน';
    	}  

    	if (!$this->customer->login($this->customer->getEmail() , $this->request->post['oldpassword'])) {
      		$this->error['warning'] = 'password ปัจจุบันไม่ถูกต้อง';
    	}
	
		if (!$this->error) {
			echo "<script>alert('เปลี่ยน password เรียบร้อย');</script>";
	  		return true;
		} else {
	  		return false;
		}
