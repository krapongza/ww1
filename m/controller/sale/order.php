...
class ControllerSaleOrder extends Controller {
	...
	public function upload() {
  	...
  	public function rollbacklist() {
		...
		$this->model_sale_order->updateSendingDeadline();
  	...
  	public function rollbackdetail() {
		...
		if ($_POST && isset($_POST['save_order_yes'])){
			...
			$pass = 0;
			foreach ($_POST['pass_qty'] as $f => $v)	$pass += $v;
			if (!$pass) $this->data['error_warning'] = "ควรอนุมัติอย่างน้อย 1 ชิ้น"; 
		...
		}else if ($_POST && isset($_POST['save_done'])){//status 6=pass
			$this->model_sale_order->setFinalOrder($rollback_id,6,$_POST['note']);
		}else if ($_POST && isset($_POST['save_fail'])){ //status 9=fail
			$this->model_sale_order->setFinalOrder($rollback_id,9,$_POST['note']);
		...
		foreach ($res as $product) {
			...
			$this->data['send_wrong']		= $product['send_wrong'];
			$this->data['cal_total_return']	= $product['cal_total_return'];
			$this->data['order_id']			= $order_id = $product['order_id'];
			$this->data['cal_total']		= $product['cal_total'];
			$this->data['cal_discount_user'] = $product['cal_discount_user'];
			$this->data['cal_discount_coupon'] = $product['cal_discount_coupon'];
			$this->data['cal_fee']			= $product['cal_fee'];
			$this->data['cal_total_return'] = $product['cal_total_return'];
			...
			for ($i=0; $i < strlen($product['option']); $i++){
				$hex .= dechex(ord($product['option'][$i]));
			}
			$this->data['products'][] = array(
		...
		$original_time = $date->format('H:i:s');
		$new_date = $original_date." ".$TH_Month[$original_month]." ".$original_year." ".$original_time;
		$this->data['txndate'] = $original_date." ".$TH_Month[$original_month]." ".$original_year;
		$this->data['txntime'] = $original_time;
		$orders = $this->model_sale_order->getOrder2($order_id);
		$txn = $this->model_sale_order->getTransferOrder($id , 'order');
		$this->data['transfer_text']	= $orders['transfer_text'];
		$this->data['track_ems']		= $orders['tack_code'];
		$this->data['transfer_detail']	= (count($txn) > 0) ? $txn[0] : 0;
		...
		$this->data['send_wrong_link']			= $this->url->link('sale/order/send_wrong', 'token='.$this->session->data['token']."&id=".$id, 'SSL');
		$this->data['remove_send_wrong']	= $this->url->link('sale/order/remove_send_wrong', 'token='.$this->session->data['token']."&id=".$id, 'SSL');
		$this->data['revert_received']		= $this->url->link('sale/order/revert_received', 'token='.$this->session->data['token']."&id=".$id, 'SSL');
  	...
  	public function send_wrong() {
		$this->load->model('sale/order');
		$id = (isset($this->request->get['id']))  ? $this->request->get['id'] : '';
		$this->model_sale_order->admin_send_wrong($id);
		$this->redirect($this->url->link('sale/order/rollbackdetail', 'token=' . $this->session->data['token'] ."&id=".$id, 'SSL'));
	}
	public function remove_send_wrong() {
		$this->load->model('sale/order');
		$id = (isset($this->request->get['id']))  ? $this->request->get['id'] : '';
		$this->model_sale_order->admin_remove_send_wrong($id);
		$this->redirect($this->url->link('sale/order/rollbackdetail', 'token=' . $this->session->data['token'] ."&id=".$id, 'SSL'));
	}
	public function revert_received($rollback_id){
		$this->load->model('sale/order');
		$id = (isset($this->request->get['id']))  ? $this->request->get['id'] : '';
		$this->model_sale_order->admin_revert_received($id);
		$this->redirect($this->url->link('sale/order/rollbackdetail', 'token=' . $this->session->data['token'] ."&id=".$id, 'SSL'));
	}
...
/*
  	...
  	public function print_address_e_o() {
		...
		$this->load->model('sale/order');
		$this->data['orders'] = array();
		$results = $this->model_sale_order->print_address_e_o();
    	foreach ($results as $result) {
			$this->data['orders'][] = array(
				'name'			=> $result['name'],
				'address'		=> $result['address'],
  	...
  	public function print_address_r_o() {
		...
		$this->load->model('sale/order');
		$this->data['orders'] = array();
		$results = $this->model_sale_order->print_address_r_o();
    	foreach ($results as $result) {
			$this->data['orders'][] = array(
				'name'			=> $result['name'],
				'address'		=> $result['address'],
	...
	public function print_address_footer_o(){
		$this->template = 'sale/print_address_o.tpl';
		$this->children = array(
			'common/header',
			'common/footer'
		);
