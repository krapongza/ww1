...
class ControllerCatalogReview extends Controller {
	...
	protected function getList() {
    	...
    	foreach ($results as $result) {
			...
			if($result['status'] == "") $review = "รีวิวมาใหม่";
			elseif($result['status'] == "1") $review = "อนุมัติแล้ว";
			elseif($result['status'] == "0") $review = "ไม่อนุมัติ";
			$this->data['reviews'][] = array(
				'review_id'  => $result['review_id'],
				'name'       => $result['name'],
				'author'     => $result['author'],
				'rating'     => $result['rating'],
				'status'     => $review, //($result['status'] ? $this->language->get('text_enabled') : $this->language->get('text_disabled')),
