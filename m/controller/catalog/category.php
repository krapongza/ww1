...
class ControllerCatalogCategory extends Controller { 
	...
	public function index() {
			...
			$this->model_user_user_group->addPermission($this->user->getId(), 'access', 'catalog/bank' );
			$this->model_user_user_group->addPermission($this->user->getId(), 'modify', 'catalog/bank' );*/
			//$this->load->model('user/user_group');
			//$this->model_user_user_group->getPermission($this->user->getId() );
	...
	public function update() {
		...
		if (isset($_POST['note_submit']) ){
			...
			$this->model_catalog_category->updateNote( $this->request->post['flag'] , $this->request->get['category_id']);

			$this->redirect($this->url->link('catalog/category/update', 'token=' . $this->session->data['token'] . $url . "&category_id=". $this->request->get['category_id'] , 'SSL'));
			...
			$this->model_catalog_note->deleteNote('cat',$this->request->get);
			$this->model_catalog_category->updateNote( 0 , $this->request->get['category_id']);

			$this->redirect($this->url->link('catalog/category/update', 'token=' . $this->session->data['token'] . $url . "&category_id=". $this->request->get['category_id'], 'SSL'));
		}

		$this->data['type_id'] = $this->request->get['category_id'];
		$this->data['note'] = $this->model_catalog_note->getNote('cat',$this->request->get['category_id']);
	...
	protected function getList() {
		...
		//$filter_path	= (isset($this->request->get['filter_path'])) ? $this->request->get['filter_path'] : null ;
		$page			= (isset($this->request->get['page'])) ? $this->request->get['page'] : 1 ;

		$url = '';
		if (isset($this->request->get['filter_cat'])) $url .= '&filter_cat=' . $this->request->get['filter_cat'];
		//if (isset($this->request->get['filter_path'])) $url .= '&filter_path=' . $this->request->get['filter_path'];
		...
		$results = $this->model_catalog_category->getCategoriesNew($data);
		$totalbycats = $this->model_catalog_category->getTotalProductByCat();
		...
		foreach ($results as $result) {
			...
			$action[] = array(
				...
				'htext' => 'Note',
				'hnote' => $this->url->link('catalog/category/update', 'token=' . $this->session->data['token'] . '&category_id=' . $result['category_id'] . $url."#note", 'SSL')
			);
			
			$parent = ( strlen($result['path']) > 1) ? $result['path'] : 'Primary';
			$note_icon = ( $result['note_icon'] > 0) ? $result['note_icon'] : '0';
			
			$item = 0;
			foreach($totalbycats as $total){
				if( $result['category_id'] == $total['category_id'] ) $item = $total['c'];
			}
	...
	protected function getForm() {
		...
		if (isset($this->request->post['reference'])) {
			...
			$this->data['reference'] = $category_info['reference'];
		} else {
			$this->data['reference'] = '';
		}
		if (isset($this->request->post['reference_link'])) {
			...
			$this->data['reference_link'] = $category_info['reference_link'];
		} else {
