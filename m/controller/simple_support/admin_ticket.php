	...
	class ControllerSimpleSupportAdminTicket extends Controller {
		...
		public function insert() {
			...
			if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validateForm()) {
				...
				$this->redirect($this->url->link('simple_support/admin_ticket', 'token=' . $this->session->data['token'] . $url, 'SSL'));
			}
		...
		public function getList() {
			...
			if($this->config->get('simple_support_ticket_permission')) {
				...
				if(in_array($this->user->getId(), $simple_support_ticket_permission)) {
					...
					$this->data['view_all_ticket'] = $this->url->link('simple_support/admin_ticket', 'token=' . $this->session->data['token'] . '&view_all=1', 'SSL');
				}	
			}
			
			$this->data['insert'] = $this->url->link('simple_support/admin_ticket/insert', 'token=' . $this->session->data['token'] . $url, 'SSL');
			$this->data['delete'] = $this->url->link('simple_support/admin_ticket/delete', 'token=' . $this->session->data['token'] . $url, 'SSL');	
			...
			$ticket_total = $this->model_simple_support_admin_ticket->getTotalTickets($data);
			
			$results = $this->model_simple_support_admin_ticket->getTickets($data);
			...
			foreach ($results as $result) {
				...
				$action[] = array(
					...
					'href' => $this->url->link('simple_support/admin_ticket/update', 'token=' . $this->session->data['token'] . '&simple_support_ticket_id=' . $result['simple_support_ticket_id'] . $url, 'SSL')
				);
			...
			$this->data['sort_ticket'] = $this->url->link('simple_support/admin_ticket', 'token=' . $this->session->data['token'] . '&sort=sst.ticket_id' . $url, 'SSL');
			$this->data['sort_subject'] = $this->url->link('simple_support/admin_ticket', 'token=' . $this->session->data['token'] . '&sort=sst.subject' . $url, 'SSL');
			$this->data['sort_ticket_status'] = $this->url->link('simple_support/admin_ticket', 'token=' . $this->session->data['token'] . '&sort=sst.simple_support_ticket_status_id' . $url, 'SSL');
			$this->data['sort_status'] = $this->url->link('simple_support/admin_ticket', 'token=' . $this->session->data['token'] . '&sort=sst.status' . $url, 'SSL');
			$this->data['sort_date_added'] = $this->url->link('simple_support/admin_ticket', 'token=' . $this->session->data['token'] . '&sort=sst.date_added' . $url, 'SSL');
			$this->data['sort_date_modified'] = $this->url->link('simple_support/admin_ticket', 'token=' . $this->session->data['token'] . '&sort=sst.date_modified' . $url, 'SSL');
			...
			$this->template = 'simple_support/admin_ticket_list.tpl';
			$this->children = array(
				'common/header',
				'common/footer'
			);
		...
		public function getForm() {
			...
			$this->data['departments'] = $this->model_simple_support_admin_ticket->getDepartments();
			
			if (!isset($this->request->get['simple_support_ticket_id'])) {
				$this->data['action'] = $this->url->link('simple_support/admin_ticket/insert', 'token=' . $this->session->data['token'] . $url, 'SSL');
			} else {
				$this->data['action'] = $this->url->link('simple_support/admin_ticket/update', 'token=' . $this->session->data['token'] . '&simple_support_ticket_id=' . $this->request->get['simple_support_ticket_id'] . $url, 'SSL');
			}
			
			$this->data['cancel'] = $this->url->link('simple_support/admin_ticket', 'token=' . $this->session->data['token'] . $url, 'SSL');
			...
			$this->template = 'simple_support/admin_ticket_form.tpl';
			$this->children = array(
				'common/header',
				'common/footer'
			);
