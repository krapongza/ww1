...
class ModelLocalisationOrderStatus extends Model {
	...
	public function getOrderStatuses($data = array()) {
      	...
      	if ($data) {
			$sql = "SELECT * FROM my_order_status AS u WHERE  u.order > 0 ORDER BY u.name";	
			...
			if (!$order_status_data) {
				$query = $this->db->query("SELECT * FROM my_order_status AS u WHERE  u.order > 0 ORDER BY u.name");
	...
	public function getReturnStatuses() {
			$sql = "SELECT * FROM my_return_status AS u  ";	
			$query = $this->db->query($sql);
			return $query->rows;
